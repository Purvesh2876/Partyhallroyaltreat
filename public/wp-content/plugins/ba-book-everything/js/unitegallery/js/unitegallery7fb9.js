...
return(false);if(objSize1.left>objSize2.right)
return(false);if(objSize1.bottom<objSize2.top)
return(false);if(objSize1.top>objSize2.bottom)
return(false);return(true);}
...
function onMouseMove(event){if(g_temp.isControlEnabled==false)
...
return(true);g_temp.touchEnabled=true;});jQuery("body").mousemove(onMouseMove);}
...
imageAlt=objItem.description;if(currentImage==urlImage&&isForce!==true){var objImage=objItemWrapper.children("img");objImage.attr("alt",imageAlt);if(objItem.imageWidth==0||objItem.imageHeight==0){g_gallery.checkFillImageSize(objImage,objItem);}
...
else{var objImage=g_functions.placeImageInsideParent(urlImage,objItemWrapper,objItem.imageWidth,objItem.imageHeight,scaleMode,objPadding);objImage.attr("alt",imageAlt);if(objItem.isBigImageLoaded==true){objImage.fadeTo(0,1);hidePreloader(objPreloader);if(slideType!="image"&&g_options.slider_video_constantsize==true)
...
function UGTextPanel(){var t=this;var g_objPanel,g_objParent,g_objTitle,g_objDesc;var g_objBG,g_objTextWrapper,g_gallery;var g_functions=new UGFunctions(),g_optionsPrefix="";var g_options={textpanel_align:"bottom",textpanel_margin:0,textpanel_text_valign:"middle",textpanel_padding_top:10,textpanel_padding_bottom:10,textpanel_height:null,textpanel_padding_title_description:5,textpanel_padding_right:11,textpanel_padding_left:11,textpanel_fade_duration:200,textpanel_enable_title:true,textpanel_enable_description:true,textpanel_title_as_link:false,textpanel_title_link_newtab:true,textpanel_enable_bg:true,textpanel_bg_color:"#000000",textpanel_bg_opacity:0.4,textpanel_title_color:null,textpanel_title_font_family:null,textpanel_title_text_align:null,textpanel_title_font_size:null,textpanel_title_bold:null,textpanel_css_title:{},textpanel_desc_color:null,textpanel_desc_font_family:null,textpanel_desc_text_align:null,textpanel_desc_font_size:null,textpanel_desc_bold:null,textpanel_css_description:{},textpanel_desc_style_as_title:false,textpanel_bg_css:{}};var g_temp={isFirstTime:true,setInternalHeight:true,lastTitleBottom:0,lastDescHeight:0};function positionElementsTop(animateHeight,startY){if(!startY)
...
this.loadAPI=function(isExternalLoad){if(g_ugWistiaAPI.isAPILoaded==true)
...
this.loadAPI=function(isExternalLoad){if(g_ugSoundCloudAPI.isAPILoaded==true)
...
function UGHtml5MediaAPI(){this.isAPILoaded=false;var t=this,g_objThis=jQuery(this),g_intHandle;var g_player;this.events={START_PLAYING:"start_playing",STOP_PLAYING:"stop_playing",VIDEO_ENDED:"video_ended"};this.loadAPI=function(isExternalLoad){if(g_ugHtml5MediaAPI.isAPILoaded==true)
...
function putVideoActually(divID,data,width,height,isAutoplay){g_player=null;g_isPlayerReady=false;var htmlID=divID+"_video";var htmlAutoplay="";if(isAutoplay&&isAutoplay===true)
...
this.pause=function(){t.doCommand("pause");}
this.play=function(){t.doCommand("play");}}
function UGVimeoAPI(){this.isAPILoaded=false;var t=this,g_objThis=jQuery(this),g_intHandle;var g_player=null,g_isPlayerReady=false,g_lastCotnainerID,g_cueChangeAutoplay=false;this.events={START_PLAYING:"start_playing",STOP_PLAYING:"stop_playing",VIDEO_ENDED:"video_ended"};this.loadAPI=function(isExternalLoad){if(g_ugVimeoAPI.isAPILoaded==true)
...
g_prevState=state;}
this.loadAPI=function(isExternalLoad){if(g_ugYoutubeAPI.isAPILoaded==true)
...
return(1);if(g_objLightbox){var isLightboxOpened=g_objLightbox.isOpened();if(isLightboxOpened==true)
return(2);}
return(0);}
this.getGalleryID=function(){return(g_galleryID);}
function isThisGalleryForKeyAction(){if(!window.ugArrGalleriesObjects)
return(true);var numGalleries=window.ugArrGalleriesObjects.length;if(window.ugArrGalleriesObjects.length==1)
return(true);var maxScore=0;var currentScore;var currentGalleryID=t.getGalleryID();jQuery.each(window.ugArrGalleriesObjects,function(index,objGallery){var galleryID=objGallery.getGalleryID();var score=objGallery.getKeyPressScore();if(score>maxScore)
...
return(true);var keyCode=(event.charCode)?event.charCode:((event.keyCode)?event.keyCode:((event.which)?event.which:0));var wasAction=false;switch(keyCode){case 39:var isGalleryForKeyAction=isThisGalleryForKeyAction();if(isGalleryForKeyAction==true){t.nextItem();wasAction=true;}
break;case 37:var isGalleryForKeyAction=isThisGalleryForKeyAction();if(isGalleryForKeyAction==true){t.prevItem();wasAction=true;}
break;default:wasAction=false;break;}
...
this.isGalleryInsideScreen=function(){if(g_temp.isFreestyleMode==true)
return(false);if(!g_objSlider)
return(false);var isInside=g_functions.isElementInsideScreen(g_objWrapper);return(isInside);}
